...
def extract_frequency_grid(gyre_files, output_file='pulsationGrid.tsv', expected_nr_orders=60, parameters=['rot', 'Z', 'M', 'logD', 'aov', 'fov', 'Xc']):
    """
    Extract frequencies from each GYRE file in the given directory and write them to 1 large file.
    ------- Parameters -------
    gyre_files: string
...
def all_freqs_from_summary(GYRE_summary_file, parameters, expected_nr_orders):
    ...
    ------- Returns -------
    line: string
        Line containing all the model parameters and pulsation frequencies
        of the GYRE summary file.
    """
...
def construct_theoretical_freq_pattern(pulsationGrid_file, observations_file, method_build_series, highest_amplitude_pulsation=None,
                                        which_observable='frequencies', output_file=f'theoretical_frequency_patterns.tsv'):
    """
    Construct the theoretical frequency pattern for each each model in the grid (each model is a row in 'pulsationGrid_file'), which correspond to the observed pattern.
    ------- Parameters -------
    pulsationGrid_file: string
        path to file containing input parameters of the models, and the pulsation frequencies of those models (as generated by function 'extract_frequency_grid').
    ...
    which_observable: string
        observable used in the frequency
    output_file: string
        Name (can include a path) for the file containing all the pulsation frequencies of the grid.
    """
    # Read in the files with observed and theoretical frequencies as pandas DataFrames
    ObsVal = pd.read_table(observations_file, delim_whitespace=True, header=0)
    ObsFreq = np.asarray(ObsVal.iloc[0])      # frequencies
    ObsErr_f = np.asarray(ObsVal.iloc[1]*4)   # times 4 to take correlation structure of the data into account (see Moravveji et al. 2016)
    ObsPer = np.asarray(ObsVal.iloc[2])       # periods
    ObsErr_P = np.asarray(ObsVal.iloc[3]*4)   # times 4 to take correlation structure of the data into account (see Moravveji et al. 2016)

    TheoFile = pd.read_table(pulsationGrid_file, delim_whitespace=True, header=0)
    ...
    if which_observable=='frequencies':
        theo_pattern_func = partial(theoretical_pattern_from_dfrow, method_build_series=method_build_series, Obs=ObsFreq,
                                    ObsErr=ObsErr_f, highest_amp_puls=highest_amplitude_pulsation, which_observable='frequencies')
    elif which_observable=='periods':
        theo_pattern_func = partial(theoretical_pattern_from_dfrow, method_build_series=method_build_series, Obs=ObsPer,
                                    ObsErr=ObsErr_P, highest_amp_puls=highest_amplitude_pulsation, which_observable='periods')

    # Send the rows of the dataframe iteratively to a pool of processors to get the theoretical pattern for each model
    p = multiprocessing.Pool()
    freqs = p.imap(theo_pattern_func, TheoFile.iterrows())

    # Make the output file directory and write the file
    Path(Path(output_file).parent).mkdir(parents=True, exist_ok=True)
    with open(output_file, 'w') as tsvfile:
        writer = csv.writer(tsvfile, delimiter='\t')
        header = list(TheoFile.loc[:,:'Xc'].columns)
        for i in range(1, len(ObsFreq)+1):
            f = 'f'+str(i)
            f.strip()
            header.append(f)
...
################################################################################
def theoretical_pattern_from_dfrow(summary_grid_row, method_build_series, Obs=[], ObsErr=[], highest_amp_puls=None, which_observable=''):
    """
