...
for grid in config.grids:
    ...
    if not Path(output_file).is_file():
        ffg.extract_frequency_grid(f'{config.grid_parent_directory}/{grid}/GYRE_out/rot{config.rotation_gyre}_k{config.kval}m{config.mval}/Zini*/*{config.subgrid}*.HDF', output_file=output_file, nr_cpu=config.nr_cpu)
    else:
        config.logger.warning(f'file already existed: {output_file}')
