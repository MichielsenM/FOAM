...
def calculate_likelihood(theory_file, observables=None, merit_function=None, obs_path=None, star_name=None, fixed_params=None, grid_parameters=None):
    ...
    missing_absolute = np.where(theory_dataframe.columns.to_series().str.contains(f'{observed_quantity}_missing'))[0]    # get the interruptions in the pattern, absolute index in dataframe
    missing_relative = np.where(theory_dataframe.filter(like=f'{observed_quantity}').columns.to_series().str.contains(f'{observed_quantity}_missing'))[0]  # get the interruptions in the pattern, index relative within pulsations
    theory_dataframe = theory_dataframe.drop(columns=theory_dataframe.columns[missing_absolute])   # Remove columns of missing frequencies
    ...
    for i in range(len(theory_puls)):
        if (theory_puls[i][0] !=-1) and (theory_puls[i][-1] !=-1):     # ignore models where one of the freqs is -1
            theory_observables = create_theory_observables_array(theory_dataframe, i, observables, missing_indices)   # make an array of the theoretical observables for each model
    ...
    combined_data = np.concatenate((np.matrix(merit_values).T,new_thetas),axis=1)  # add an additional column for MLE 'meritValues'
    df = pd.DataFrame(data=combined_data, columns=['meritValue']+['rot']+['rot_err']+grid_parameters) # put the data in a pandas DataFrame
    df = pd.merge(df, theory_dataframe.drop(list(theory_dataframe.filter(regex=f'{observed_quantity}').columns), axis=1), how='inner', on=['rot', 'rot_err']+grid_parameters)
...
def create_theory_observables_array(theory_dataframe, index, observables_in, missing_indices):
    ...
    if 'P' in observables:   # Cast to list before using asarray to prevent memory leak
        observables_out = np.asarray(list(theory_dataframe.filter(like='period').loc[index]))  # add the periods to the output list
        observables.remove('P')

    elif 'f' in observables: # Cast to list before using asarray to prevent memory leak
        observables_out = np.asarray(list(theory_dataframe.filter(like='frequency').loc[index]))  # add the frequencies to the output list
...
def create_obs_observables_array(obs_dataframe, observables):
    ...
    Returns
    ----------
    observables_out, observables_err_out: numpy array, dtype=float
    ...
    """
    missing_indices = np.where(obs_dataframe.index.isin(['f_missing']))[0] # get the interruptions in the pattern
    ...
    if len(missing_indices)!=0: obs_dataframe = obs_dataframe.drop(index='f_missing')  # remove lines indicating missing frequencies (if they are present)
    ...
    for observable in observables:
        ...
        if observable == 'logTeff': 
            ...
            logTeff = True
        obs = np.asarray(obs_dataframe[observable]) # since these columns have less entries, the dataFrame has NaNs in the empty rows
...
def merit_chi2(Yobs, obs_err, YTheo, fig_title=None, star_name=None):
    ...
    Parameters
    ----------
    Yobs, obs_err: numpy array, dtype=float
...
def merit_mahalanobis(Yobs, obs_err, YTheo, generate_output=True, fig_title=None, star_name=None):
    ...
    v_matrix = v_matrix + np.diag(obs_err**2.)
    check_matrix(v_matrix, generate_output=generate_output, fig_title=fig_title, star_name=star_name)     # check if positive definite and make figure
...
def check_matrix(v_matrix, generate_output=True, fig_title='Vmatrix', star_name=None):
    ...
    star_name: string
        The name of the analysed star, for file naming purposes.
    """
    if np.all(np.linalg.eigvals(v_matrix) > 0)==False: # If all eigenvalues are >0, it is positive definite
    ...
    if generate_output is True:
        ...
        with file_path.open("a") as file:
            file.write(f'{fig_title} \t {np.log(np.linalg.det(np.exp(kk)*v_matrix))-kk*v_matrix.shape[0]:.2f} \t {np.linalg.cond(v_matrix):.2f} \n ')


        im = plt.imshow(v_matrix*10**4, aspect='auto', cmap='Reds') # Do *10^4 to get rid of small values, and put this in the colorbar label
