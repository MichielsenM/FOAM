...
## The observational data
 ...
 Splitting the pattern in multiple parts that each hold more than one frequency works for the different setups. However, the occurence an isolated frequency in between missing frequencies (f_(n-1), f_missing, f_n, f_missing, f_(n+1), f_(n+2)...) should be avoided when period spacings are used as seismic observables (`observable_seismic=['dP']`, see [Pipeline configuration](./Configuration.md)), since they won't have a period spacing associated with them, and the numerical values for the statistical criteria will have an offset compared to their actual value (constant and equal for all models in the setup). Isolated frequencies pose no problem and can be used in case periods and/or frequencies are used as seismic observables (`observable_seismic=['P', 'f']`).
